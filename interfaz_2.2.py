...
# =========================
# 🧠 CARGAR MODELO (Robusto con caché de TensorFlow)
# =========================
@st.cache_resource
def load_model():
    """Descarga y carga el modelo usando el sistema de caché de TensorFlow"""
    
    st.info("📥 Verificando modelo...")
    
    try:
        # tf.keras.utils.get_file:
        # - Descarga si no existe en caché
        # - Guarda en ~/.keras/models/
        # - Reutiliza en futuras ejecuciones
        # - Muestra progreso en consola
        model_path = tf.keras.utils.get_file(
            fname=MODEL_NAME,
            origin=MODEL_URL,
            cache_subdir="models",
            file_hash=None  # Opcional: añade hash SHA256 para verificar integridad
        )
        
        st.success(f"✅ Modelo listo: {os.path.basename(model_path)}")
        return tf.keras.models.load_model(model_path)
        
    except Exception as e:
        st.error(f"❌ Error al cargar el modelo: {e}")
        st.warning("💡 Verifica que:")
        st.warning("   1. El enlace de GitHub Releases sea DIRECTO (clic derecho → Copiar enlace)")
        st.warning("   2. El release sea PÚBLICO")
        st.warning("   3. El nombre del archivo coincida: MODELO_FINAL.keras")
        st.stop()
...
if img_file:
    ...
    # 🔥 IMPORTANTE: Mismo tamaño que el entrenamiento
